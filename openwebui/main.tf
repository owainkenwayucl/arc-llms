...
resource "harvester_virtualmachine" "vm" {
  ...
  name                 = "${var.username}-openwebui-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  namespace            = local.namespace
  ...
  hostname        = "${var.username}-openwebui-${format("%02d", count.index + 1)}-${random_id.secret.hex}"
  ...
  tags = {
    ...
    condenser_ingress_endpt_hostname = "openwebui"
    condenser_ingress_endpt_port = 5000
